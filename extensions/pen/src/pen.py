...
def stamp(target):
    ...
    stage.add_paint(PEN_PAINT, lambda disp: disp.draw(
        image, x, y, width, height, key = True, palette = palette
    ))
...
def pen_goto(target, *args, **kwargs):
    ...
    stage.add_paint(PEN_PAINT, lambda disp: disp.linex(
        x, y, nx, ny, size, color565(*color)
    ))
...
def set_color(target, color = None, hue = None, saturation = None, brightness = None):
    if color == None:
        ...
        if hue != None: h = hue % 100 / 100
        if saturation != None: s = saturation % 100 / 100
        if brightness != None: v = brightness % 100 / 100
