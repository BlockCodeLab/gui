...
javascriptGenerator2["event_whenbroadcastreceived"] = (block) => {
  let code = "";
  const messageName = javascriptGenerator2.variableDB_.getName(block.getFieldValue("BROADCAST_OPTION"), ScratchBlocks.Variables.NAME_TYPE);
...
javascriptGenerator7.scrub_ = function(block, code) {
  ...
  if (!block.outputConnection || !block.outputConnection.targetConnection) {
    ...
    comment = ScratchBlocks2.utils.wrap(comment, javascriptGenerator7.COMMENT_WRAP - 3);
    if (comment) {
    ...
    for (var i = 0;i < block.inputList.length; i++) {
      if (block.inputList[i].type == ScratchBlocks2.INPUT_VALUE) {
